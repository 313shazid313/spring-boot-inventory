...
const EditProfile = () => {
  ...
  const {
    ...
    error,
  } = useSingleEmployeeQuery(storageData.id);
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    try {
      await updateEmployee({ id: storageData.id, ...element }).unwrap();
      toast.success("Employee updated successfully!");
      navigate("/dashboard/profile"); // redirect to employee list (or wherever)
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <div>
        <p className="text-2xl font-bold mb-6 text-center">Update Profile</p>
